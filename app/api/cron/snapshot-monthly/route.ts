...
/**
 * GET /api/cron/snapshot-monthly
 * Runs on the 1st of each month at 03:30 UTC (09:00 IST).
 * Pulls data from all configured APIs and writes normalised metrics to
 * metric_snapshots + youtube_snapshots for proper YoY comparison.
 */
...
export async function GET(request: NextRequest) {
  ...
  if (results.length > 0) {
    await storeMetricSnapshots(results as Parameters<typeof storeMetricSnapshots>[0], 'monthly', startDate, endDate, serviceClient);
  }

  // Store leads snapshot from Supabase
  try {
    await storeLeadsSnapshot('monthly', startDate, endDate, serviceClient);
